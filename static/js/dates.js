        ...
        body.innerHTML = dates.map(d => {
            ...
            return `
                <tr class="${isPast ? 'opacity-60' : ''} border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    <td class="p-4 font-mono text-[10px] text-slate-400 uppercase">${caseNo}</td>
                    <td class="p-4 text-sm font-bold text-blue-600">${defendant}</td>
                    <td class="p-4 text-xs font-medium text-slate-600">${d.paper?.type || '--'}</td>
                    <td class="p-4"><span class="px-2 py-0.5 rounded text-[10px] font-bold ${badgeColor}">${d.party}</span></td>
                    <td class="p-4">
                    ...
                    <td class="p-4 text-xs text-slate-600 italic">${d.optional_text || ''}</td>
